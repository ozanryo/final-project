...
public class controllerReceive {
    ...
    public String receiveUpdateProfileRespondFromDB() throws IOException, TimeoutException {
        ConnectionFactory conFac = new ConnectionFactory();
        conFac.setHost("localhost");
        Connection con = conFac.newConnection();
        Channel ch = con.createChannel();

        ch.queueDeclare("respondUpdateProfile", false, false, false, null);
        GetResponse getResponse;
        do {
            getResponse = ch.basicGet("respondUpdateProfile", true);
        } while (getResponse==null);
